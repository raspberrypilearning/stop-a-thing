...
Start by adding your variables and the basic functions: `preload()`, `setup()`, and an empty `draw()` function.

In the `preload()` function, load the image `robot.png` so it’s ready before the program starts.
